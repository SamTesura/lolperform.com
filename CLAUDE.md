...
- Every stat needs its sample size and confidence treatment. Champions below the threshold belong in the explicit
  _Unranked_ section, never graded.
- `astroCompressHTML` in `.prettierrc.json` must mirror `compressHTML` in `apps/web/astro.config.mjs`. Both sit on
  their defaults today (`'jsx'`, Astro 7's), and the pin is explicit so the pair can't drift silently — a mismatch
  makes the formatter move whitespace the compiler treats differently, which is how words get joined on the page.
  `pnpm format:check` runs in CI so a prettier or plugin bump can no longer land as churn in an unrelated PR.
